...
function Test-UserRights {
    ...
    $validation = Test-ExclusiveGroupMembership -CurrentValue $current -ExpectedSids @("S-1-5-32-546")
    Write-TestResult -ControlID "2.2.16" -ControlName "Deny access from network" -Compliant $validation.Compliant -CurrentValue (Convert-SidsToNames $current) -ExpectedValue "SOLO Guests" -Details $validation.Reason
    ...
    $validation = Test-ExclusiveGroupMembership -CurrentValue $current -ExpectedSids @("S-1-5-32-546")
    Write-TestResult -ControlID "2.2.19" -ControlName "Deny log on locally" -Compliant $validation.Compliant -CurrentValue (Convert-SidsToNames $current) -ExpectedValue "SOLO Guests" -Details $validation.Reason
